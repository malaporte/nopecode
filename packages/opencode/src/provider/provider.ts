...
import { OpenAICompatibleChatLanguageModel } from "./sdk/copilot/chat/openai-compatible-chat-language-model"
...
export namespace Provider {
  ...
  export interface Interface {
    ...
    readonly db: () => Effect.Effect<Record<ProviderID, Info>>
    ...
    Effect.gen(function* () {
      ...
      const cache = yield* InstanceState.make<State>(() =>
        Effect.gen(function* () {
          ...
          const kiroModels = yield* Effect.gen(function* () {
            const ctx = yield* InstanceState.context
            return yield* Effect.promise(() => loadKiroModels(ctx.directory).catch(() => KIRO_MODELS))
          })
          ...
          for (const plugin of plugins) {
            ...
            let pluginAuth = yield* auth.get(providerID).pipe(Effect.orDie)
            if (!pluginAuth) {
              // loader may auto-provision credentials (e.g. kiro-cli import) —
              // call it, then re-check.
              yield* Effect.promise(() =>
                plugin.auth!.loader!(() => Auth.get(providerID) as any, database[plugin.auth!.provider]).catch(
                  () => undefined,
                ),
              )
              pluginAuth = yield* auth.get(providerID).pipe(Effect.orDie)
      ...
      const db = Effect.fn("Provider.db")(() => InstanceState.use(cache, (s) => s.database))
      ...
      return Service.of({ list, db, getProvider, getModel, getLanguage, closest, getSmallModel, defaultModel })
    }),
  )
